...
async def test_dummy_auth():
    cache = Mock()
    try:
        await DummyAuth(cache).auth("Token {}".format(options.dummy_auth))
    except BypassAuth as bypass_auth:
        user = bypass_auth.args[0]
    assert user.user_id == 0
    assert user.is_active
    with pytest.raises(auth.UserNotFound):
        await DummyAuth(cache).auth("Foobar")
    with pytest.raises(auth.UserNotFound):
        await DummyAuth(cache).auth(None)
...
@pytest.mark.asyncio
async def test_auth(mock_auth, mock_cache):
    mock_cache.side_effect = KeyError
    token = TEST_TOKEN
    ret = sentinel.ret
    mock_auth.return_value = ret
    cache = Mock()
    assert (await Auth(cache).auth(token)) is ret
    mock_auth.assert_called_once_with(token)
    mock_cache.assert_called_once_with(cache, token)


@pytest.mark.asyncio
async def test_auth_cache_called(mock_auth, mock_cache):
    token = TEST_TOKEN
    ret = sentinel.ret
    mock_cache.return_value = ret
    assert (await Auth(sentinel.cache_backend).auth(token)) is ret
    mock_cache.assert_called_once_with(sentinel.cache_backend, token)
    mock_auth.assert_not_called()


@pytest.mark.asyncio
async def test_auth_cache(mock_auth, cache):
    token = TEST_TOKEN
    ret = auth.User(0, True, 123, 456)
    mock_auth.return_value = ret
    assert (await Auth(cache).auth(token)) == ret
    mock_auth.assert_called_once_with(token)
...
@pytest.mark.asyncio
async def test_user_not_found(mock_auth, cache):
    mock_auth.side_effect = auth.UserNotFound
    auth_header = "Token foobar"
    with pytest.raises(auth.UserNotFound):
        await Auth(cache).auth(auth_header)


@pytest.mark.asyncio
async def test_auth_request(mocker):
    fetch_mock = Mock()
    ...
    response = await auth.AccountingServerAuth.request_auth(token)
...
@pytest.mark.gen_test
def test_auth_send_request_error_propagation(app, http_client, auth_server, auth_path):
    auth_server.add_response(services.Request('POST', auth_path),
...
@pytest.mark.gen_test
def test_auth_returns_user_object(app, http_client, auth_server, auth_path):
...
@pytest.mark.gen_test
def test_auth_returns_inactive_user_object(app, http_client, auth_server, auth_path):
